...
fu! s:parse_cmdline(cmdline)
    " TODO: Though it wouldn't make a lot of sense, allow -- to appear at end
    " of line...
	let m = matchlist(a:cmdline, '^\s*\(\%(--\?[a-zA-Z0-9_]\+\)\%(\s\+--\?[a-zA-Z0-9_]\+\)*\)\%(\s\+--\s\+\)\?\(.*\)$')
...
fu! s:refresh(opts)
    if a:opts !~ '^\s*\%(--\?[a-zA-Z0-9_]\+\)\%(\s\+--\?[a-zA-Z0-9_]\+\)*\s*$'
        echoerr "Invalid arguments specified for Refresh command: ..."
    endif
